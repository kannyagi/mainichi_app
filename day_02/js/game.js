...
const FALLBACK_SHARE_URL = 'https://example.com/mainichi/day_02';
const SONGS_PER_SOURCE = 30;
const RECENT_TRACKS_STORAGE_KEY = 'intro-don-recent-track-ids';
const RECENT_TRACKS_LIMIT = 120;
...
async function fetchSongsByGenre(genre) {
  const sources = getGenreSearchSources(genre);
  const songGroups = await Promise.all(sources.map(fetchSongsFromSource));
  return dedupeSongs(songGroups.flat());
}

function getGenreSearchSources(genre) {
  const presets = {
    jpop: [
      { term: 'japanese pop', country: 'jp' },
      { term: 'j-pop', country: 'jp' },
      { term: '邦楽 ヒット', country: 'jp' },
      { term: 'japanese 90s', country: 'jp' },
    ],
    anime: [
      { term: 'anime songs', country: 'jp' },
      { term: 'アニメソング', country: 'jp' },
      { term: 'anison', country: 'jp' },
    ],
    kpop: [
      { term: 'kpop', country: 'us' },
      { term: 'k-pop hits', country: 'us' },
      { term: 'korean pop', country: 'us' },
    ],
    pop: [
      { term: 'pop hits', country: 'us' },
      { term: 'top pop', country: 'us' },
      { term: 'today pop', country: 'us' },
    ],
    rock: [
      { term: 'rock hits', country: 'us' },
      { term: 'classic rock', country: 'us' },
      { term: 'japanese rock', country: 'jp' },
    ],
    hiphop: [
      { term: 'hip hop', country: 'us' },
      { term: 'rap hits', country: 'us' },
      { term: 'japanese hip hop', country: 'jp' },
    ],
    classic: [
      { term: '歌謡曲', country: 'jp' },
      { term: '昭和歌謡', country: 'jp' },
      { term: 'japanese 80s', country: 'jp' },
      { term: 'japanese 90s', country: 'jp' },
      { term: 'oldies', country: 'us' },
      { term: '70s hits', country: 'us' },
    ],
  };

  if (genre.id === 'random') {
    const candidates = Object.entries(presets)
      .filter(([id]) => id !== 'random')
      .flatMap(([, sources]) => sources);
    return shuffle(candidates).slice(0, 6);
  }

  return presets[genre.id] || [{ term: genre.term, country: genre.country }];
}

async function fetchSongsFromSource(source) {
  const url = `https://itunes.apple.com/search?term=${encodeURIComponent(source.term)}&entity=song&media=music&limit=${source.limit || SONGS_PER_SOURCE}&country=${source.country}&lang=ja_jp`;
  const response = await fetch(url);

  if (!response.ok) {
    throw new Error('genre source search failed');
  }
...
function dedupeSongs(songs) {
  ...
  return songs.filter(song => {
    const trackKey = String(song.trackId);
...
function prepareGameSession(songs) {
  ...
  const queue = pickQuestionQueue(songs, totalQuestions);
...
function pickQuestionQueue(songs, count) {
  const recentTrackIds = getRecentTrackIds();
  const recentSongs = [];
  const freshSongs = [];

  songs.forEach(song => {
    if (recentTrackIds.has(String(song.trackId))) {
  ...
  const prioritized = [...shuffle(freshSongs), ...shuffle(recentSongs)];
  return takeDiverseSongs(prioritized, count);
}

function takeDiverseSongs(candidates, count) {
  const picked = [];
  ...
  for (const song of candidates) {
    ...
    const trackKey = String(song.trackId);
    const artistKey = getSongArtistLabel(song).toLowerCase();

    if (usedTrackIds.has(trackKey) || usedArtists.has(artistKey)) {
      continue;
    }

    picked.push(song);
    usedTrackIds.add(trackKey);
...
function getRecentTrackIds() {
  try {
    const stored = JSON.parse(localStorage.getItem(RECENT_TRACKS_STORAGE_KEY) || '[]');
...
function rememberRecentTracks(queue) {
  try {
    const newIds = queue.map(song => String(song.trackId));
    const existing = JSON.parse(localStorage.getItem(RECENT_TRACKS_STORAGE_KEY) || '[]');
    const retained = Array.isArray(existing) ? existing.filter(id => !newIds.includes(String(id))) : [];
    const merged = [...newIds, ...retained].slice(0, RECENT_TRACKS_LIMIT);
    localStorage.setItem(RECENT_TRACKS_STORAGE_KEY, JSON.stringify(merged));
  } catch {
    // Ignore storage failures and continue without recency tracking.
  }
}
